...
use Wangyipinglove\AliGreen\Green;
...
class AliGreen
{
    ...
    private function processSceneResult($taskResults, $title, $type)
    {
        ...
        foreach ($taskResults as $value) {
            ...
            foreach ($value->results as $v) {
                $arr[] = $this->review($v->suggestion, $v->label, $v->rate, $v->scene, isset($v->details) ? $v->details : null);
            }
        }
     ...
     * @return mixed
     */
    private function llegalWords($content)
    {
        $arr = [];
        if (!isset($content[0]->contexts)) {
            return [];
        }
        foreach ($content[0]->contexts as $key => $value) {
            $arr = $value->context;
        }
        return $arr;
    }
     ...
     * @return array
     */
    private function review($suggestion, $label, $rate, $scene, $details)
    {
        $arr = $llegal_words = [];
        if (in_array($suggestion, ['review', 'block'])) {
            if ($details) {
                $llegal_words = $this->llegalWords($details);
            }
            $arr = [
                'label' => $label,
                'rate' => $rate,
